...
func TestServeAppStaleness(t *testing.T) {
	...
	if rec := get(t, srv, "myapp"); rec.Code != http.StatusServiceUnavailable {
		t.Errorf("no result = %d, want 503", rec.Code)
	}

	st.Set("myapp", store.Result{Body: []byte(`{"desiredExecutors":4}`), DesiredExecutors: 4, PolledAt: time.Now()})
	if rec := get(t, srv, "myapp"); rec.Code != http.StatusOK {
	...
	st.Set("myapp", store.Result{Body: []byte(`{"desiredExecutors":5}`), DesiredExecutors: 5, PolledAt: time.Now()})
	if rec := get(t, srv, "myapp"); rec.Code != http.StatusOK {
		t.Errorf("recovered result = %d, want 200: %s", rec.Code, rec.Body.String())
	}

	st.Set("myapp", store.Result{NoPolicy: true, PolledAt: time.Now()})
	if rec := get(t, srv, "myapp"); rec.Code != http.StatusNotFound {
